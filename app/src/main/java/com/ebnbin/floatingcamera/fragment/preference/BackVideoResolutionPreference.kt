 ...
 * 后置摄像头视频分辨率偏好.
 */
class BackVideoResolutionPreference(backVideoProfileCustomPreferenceGroup: BackVideoProfileCustomPreferenceGroup) :
        ListPreference(backVideoProfileCustomPreferenceGroup.context) {
    init {
        key = KEY
        setDefaultValue(DEF_VALUE)
        setTitle(R.string.back_video_resolution_title)
        dialogTitle = title
        setEntriesAndValues(cameraHelper.backDevice.videoResolutionSummaries)
        ...
        backVideoProfileCustomPreferenceGroup.addPreferenceToGroup(this)
